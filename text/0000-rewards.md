...
As discussed [below](#approvals), we say a validator 𝑢 *uses* an approval vote by a validator 𝑣 on a candidate 𝑐 if the the final approving run of the elves approval loop by 𝑢 counted the vote by 𝑣 towards approving the candidate 𝑐.
```
